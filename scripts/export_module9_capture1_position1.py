"""Export the Module 9 viewer from the persisted Position 1 label artifact.
...
C0_PATH = PROJECT_ROOT / "raw_data" / (
    "Capture 1 - Position 1_XY1757012095_Z0_T0_C0.tif"
)
C1_PATH = PROJECT_ROOT / "raw_data" / (
    "Capture 1 - Position 1_XY1757012095_Z0_T0_C1.tif"
)
LABEL_PATH = PROJECT_ROOT / "outputs" / "module7_ofat_review_20260714_kmeans" / (
    "runs/field_001__variant_003/labels.npy"
)
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "module9_roi_review_capture1_position1"
OUTPUT_HTML = OUTPUT_DIR / "capture1_position1_roi_review_v2.html"
OUTPUT_MANIFEST = OUTPUT_DIR / "capture1_position1_roi_review_manifest.json"

EXPECTED_SHA256 = {
    C0_PATH: "62f229a459f041b3c24907d4b27d835b67919e8139854d5fba9db26d63a5eb21",
    C1_PATH: "dd35903c267fb8528136fbadc4e4662bc6527ff6051a5fa1390111fca31307d8",
    LABEL_PATH: "36ab719aec5b736f56deb1c44f9286b023536ccc906780bad8934f51ae2ba9af",
}
...
def _validated_pair():
    ...
    if parsed_c0 is None or parsed_c1 is None:
        raise ValueError("the fixed Position 1 TIFF filenames no longer parse")
    ...
    if validation.pair is None:
        ...
        raise ValueError(f"Position 1 TIFF pair validation failed: {issue_codes}")
    pair = validation.pair
    if (
        pair.position_key.capture != FIELD_KEY.capture
        or pair.position_key.position != FIELD_KEY.position
    ):
        raise ValueError("validated TIFF pair does not match Capture 1 + Position 1")
    return pair
...
def main() -> None:
    ...
    if labels.shape != pair.c0.frames.shape[1:]:
        raise ValueError(
            "persisted Position 1 labels do not match the validated TIFF frame shape"
        )
    ...
    result = export_interactive_roi_review_html(
        ...
        review_state,
        OUTPUT_HTML,
        title="Module 9 ROI review - Capture 1 / Position 1",
    )

    manifest = {
        "artifact": str(result.path.relative_to(PROJECT_ROOT)),
        "artifact_sha256": _sha256(result.path),
        ...
        "static_atlas_panel_count": 2 * result.frame_count,
        ...
        "review": {
            "status": field_review.status.value,
            "decision_filename": result.review_filename,
            "ready_for_explicit_manual_export": True,
